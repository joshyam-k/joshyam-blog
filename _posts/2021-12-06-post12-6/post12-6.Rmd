...
description: |
  using ggplot to gain intuition for how a seemingly hopelessly complex problem worked 
author:
  - name: Josh Yamamoto
...
So I figured I might as well just try to solve for these first few probabilities and see if I could find a problem.

If you think about it if $X,Y$ are i.i.d $\text{Unif}(0,1)$ then we can imagine each realization of and X and Y being like the x and y coordinates of a random point in the cartesian product $[0,1] \times [0,1]$. And if we want to find the probability that $Z = 1$ then we can just generate many such coordinates and then see how many of these coordinates satisfy our condition. This is a lot of words and it might still be confusing so lets make a plot. We'll write it as a function since we'll want to make more of these plots later on:
